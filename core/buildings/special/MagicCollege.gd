...
const CHOICE_UPGRADE_ID: String = "magic_college:0"
const SPEED_UPGRADE_ID: String = "magic_college:1"
...
func initialize(slot: Node, config: BuildingConfig) -> void:
	...
	_is_producing = false
	_pool.clear()
	# Basic spells pool (exclude legendary_*)
	...
	while file != "":
		if (not dir.current_is_dir()) and file.ends_with(".tres"):
			...
			if not spell_id.begins_with("legendary_"):
...
func tick(delta: float) -> Dictionary:
	...
	# Start cycle only if inputs are available (if any)
...
func _on_cycle_completed() -> void:
	if _has_upgrade(CHOICE_UPGRADE_ID):
		_enqueue_spell_choice_reward(false)
		return
	if _pool.is_empty():
		return
