extends Control
class_name HomeseekerArrivalOverlay
...
func _play_sequence() -> void:
	...
	print("[HomeseekerArrivalOverlay] play: tree_paused=%s speed_scale=%.2f" % [get_tree().paused, _prev_speed_scale])
	...
	var current_speed_scale := float(tick_manager.speed_scale) if tick_manager != null else 1.0
	print("[HomeseekerArrivalOverlay] end: tree_paused=%s speed_scale=%.2f" % [get_tree().paused, current_speed_scale])
