extends Control
class_name RewardMenuLevyBarracks

@export var building_category: int = int(BuildingConfig.BuildingCategory.LEVY_BARRACKS)
@export var menu_title: String = "Choose a levy barracks building"
    ...
    if building_registry != null:
        ...
        for c in configs:
            if c and c is BuildingConfig and (c as BuildingConfig).building_id != "":
                ids.append((c as BuildingConfig).building_id)
    return ids
