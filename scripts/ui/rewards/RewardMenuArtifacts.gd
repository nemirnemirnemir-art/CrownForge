extends Control
class_name RewardMenuArtifacts
...
func _ready() -> void:
    ...
    _open_offered_count = offered_count
...
func _get_pool_ids() -> Array[String]:
    ...
    for id in ArtifactCatalog.get_player_available_ids_sorted():
        var def := ArtifactCatalog.get_def(id)
        var is_legendary := String(def.get("rarity", "")) == ArtifactCatalog.RARITY_LEGENDARY
        if _open_legendary_only and not is_legendary:
            continue
        if artifact_core != null and bool(artifact_core.call("has_artifact", id)):
            continue
        ids.append(id)
    return ids
...
func _roll_cards() -> void:
    ...
    if title_label:
        title_label.text = "Choose a legendary artifact" if _open_legendary_only else "Choose an artifact"
    if dim:
        dim.visible = true

func _on_card_selected(artifact_id: String) -> void:
    var artifact_core: Node = _artifact_core()
